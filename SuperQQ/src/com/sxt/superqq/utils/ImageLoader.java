...
public class ImageLoader {
	...
	class ImageBean{
		 ...
		 */
		public ImageBean(String path, int width, int height,
				OnImageLoadListener listener) {
			super();
			this.path = path;
			this.width = width;
	...
	class DownloadImageTask extends AsyncTask<ImageBean, Void, ImageBean>{
		...
		protected ImageBean doInBackground(ImageBean... params) {
			...
			try {
				...
				//缓存至sd卡
				BitmapUtils.saveBitmap(bean.bitmap, getDir(bean.path));
				bean.errorMsg="图片下载成功";
			} catch (Exception e) {
				bean.errorMsg="图片下载失败";
			}finally{
		...
		protected void onPostExecute(ImageBean result) {
			...
			if(!result.errorMsg.equals("图片下载失败")){
	 ...
	 * @return
	 */
	public Bitmap displayImage(String path,int width,int height,OnImageLoadListener listener){
		//若图片缓存在内存，则直接返回该图片
		if(mCaches.get(path)!=null){
			return mCaches.get(path);
		}
		//若图片缓存在sd卡，则直接返回该图片
		Bitmap bitmap = BitmapUtils.getBitmap(getDir(path));
		if(bitmap!=null){
			return bitmap;
		}
		//创建一个ImageBean对象,    预存代码
		ImageBean bean=new ImageBean(path, width, height, listener);
...
//		File dir =Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
		path=dir.getAbsolutePath()+path.substring(path.lastIndexOf("/"));
		return path;
	}
